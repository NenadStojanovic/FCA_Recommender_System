...
for (var i = 0; i < likedMovies.length; i++) {
    $.ajax({
        ...
        success: function (response) {
            ...
            var movies = response.results;
            var firstResult = movies[0];
            $("#liked_logo_" + likedMovies[this.index].id).attr("src", "https://image.tmdb.org/t/p/w300/" + firstResult.poster_path)
        },
        error: function (response) {
            $("#liked_logo_" + likedMovies[this.index].id).css("background: red;")
        }
    });
}